...
<%@ page language="java" contentType="text/html; charset=utf-8"
    pageEncoding="utf-8"%>
    <jsp:include page="MasterPage.jsp" flush="true" />
<!DOCTYPE html>
<html>
<head>
</head>
...
<table id="tblAlumnos" class="table table-striped table-responsive-lg ">
  ...
  <tbody>
							...
							NegAlumno.listarAlumnos(alumnos);
							    		for(EntAlumno a : alumnos)
							    		{%> 
	  <tr>
      <form action="SvltEliminarEditarAlumnoAdmin" method="get">
...
<div id="modalAbm" class="modal" tabindex="-1" role="dialog">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
    	       <form name="formModal" method="post" action="SvltGuardarAlumnoAdmin">
...
$( document ).ready(function() {
	...
	//Al cargar un alumno con exito se muestra una alerta si fue ok o no
	if('<%=request.getAttribute("cargarAlumno")%>'!="null" )
		{
		var error='<%=request.getAttribute("cargarAlumno")%>'
		if(error=='ok')
			{
			sweetAlert("Buen trabajo","Alumno cargado con exito","success");
			<%request.removeAttribute("cargarAlumno");%>
			$('#hfEditar').val(null);
			}
		else
			{
	...
	//Al no cargar el alumno correctamente se recarga el form con los datos ingresados
	if("<%=request.getAttribute("alumno")%>"!="null")
		...
		{	
			...
			EntAlumno alumno=(EntAlumno)request.getAttribute("alumno");
			if(alumno!=null)
			{
				%>
				$('#txtNombre').val('<%=alumno.getNombre()%>');
				$('#txtApellido').val('<%=alumno.getApellido()%>');
				$('#txtDni').val('<%=alumno.getDni()%>');
				$('#txtLegajo').val('<%=alumno.getLegajo()%>');
				$('#txtEmail').val('<%=alumno.getEmail()%>');
				$('#txtDireccion').val('<%=alumno.getDireccion()%>');
				$('#txtLocalidad').val('<%=alumno.getIdLocalidad()%>');
				$('#txtProvincia').val('<%=alumno.getIdProvincia()%>');
				$('#txtTelefono').val('<%=alumno.getTelefono()%>');
				$('#txtFechaNac').val('<%=alumno.getFecha_nac()%>');
				<%
	...
	//Cuando se elimina un alumno se muestra una alerta
	if('<%=request.getAttribute("eliminado")%>'=='true')
		{
		sweetAlert("Buen trabajo","Alumno eliminado con exito","success");
		<%request.removeAttribute("eliminado");%>
		}
	else if('<%=request.getAttribute("eliminado")%>'=='false')
		{
		sweetAlert("Error","El alumno no pudo ser eliminado","error");
		<%request.removeAttribute("eliminado");%>
		}
	
	//abre el modal al presionar editar alumno, carga sus datos y pone el hf editando en true(para if del svlt guardar)
